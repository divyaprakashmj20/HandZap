  ...
  getPhotos() {
    ...
    axios
      .get(
        ...
        this.state.album.id
        }/photos?fields=images&access_token=${this.state.responseToken}`
      )
      .then(response => {
  ...
  render() {
    const responseFacebook = response => {
      ...
      if (response.accessToken) {
        ...
        graph.get(
          `264935813565644/picture?fields=picture`,
          function (err, res) {
            // console.log(res, "test thumbnail");
             // { picture: 'http://profile.ak.fbcdn.net/'... }
          }.bind(this)
        );

        graph.get(
          `${response.userID}?fields=picture.width(200).height(200)`,
          function (err, res) {
            // console.log(res);
             // { picture: 'http://profile.ak.fbcdn.net/'... }
            this.setState({
      ...
      <div className="App">
        ...
        {this.state.loginStatus == false && (
          <div>
            ...
            <FacebookLogin
              appId="258607481715860"
              autoLoad
              callback={responseFacebook}
              fields="name,email,picture,albums"
              // scope="public_profile,user_friends,user_actions.books"
              render={renderProps => (
                <button className="test" onClick={renderProps.onClick}>
          ...
          <div>
            ...
            <div>
              {this.state.loginResponse.albums.data.map(p => {
                var source = `https://graph.facebook.com/v3.2/${
                  p.id
        ...
        {(this.state.page == 3 || this.state.page == 2) && (
          <div>
            <div style={{ textAlign: "right", padding: "24px 50px 24px 0px" }}>
              <label
                style={{
                  cursor: "pointer",
                  fontSize: "20px",
          ...
          <div>
            ...
            <div className="slide">
              ...
              >
                <img
                  className="slideimg"
                  src={this.state.album.images[this.state.slide.clicked]}
                />
              </a>
